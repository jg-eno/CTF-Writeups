...
export const ActualGiftRouter = Router();


ActualGiftRouter.get('/', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (blogCount >= requiredBlogs) {
            res.status(200).json({ 
                message: "Congratulations! You received a special gift : apoorvctf{s1gm@_s1gm@_b0y}",
                blogCount
            });
        }
        else {
